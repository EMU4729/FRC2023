...
public class GripperSub extends SubsystemBase {
  private final Servo servoOne = new Servo(Constants.gripper.SERVO_1_ID);
  private final Servo servoTwo = new Servo(Constants.gripper.SERVO_2_ID);
  ...
  public void periodic() {
    final double valueOne = MathUtil.clamp(ShuffleControl.gripperTab.getValueOne(), 0, 1);
    final double valueTwo = MathUtil.clamp(ShuffleControl.gripperTab.getValueTwo(), 0, 1);
    // Logger.info("Gripper : Value One = " + valueOne + ", Value Two = " +
    // valueTwo);
    servoOne.set(valueOne);
    servoTwo.set(valueTwo);
  }
}
